    ...
    def Run(self):
        ...
        def bugsnag(proj, offscreenCanvas):
            ...
            if len(newErrors) < bugLow:
                graphics.DrawText(offscreenCanvas, fontBig, 29, 12, green, str(len(newErrors)))
            elif len(newErrors) > bugHigh:
                graphics.DrawText(offscreenCanvas, fontBig, 29, 12, orange, str(len(newErrors)))
            else:
            ...
            if len(ipErrors) < bugLow:
                ...
                graphics.DrawText(offscreenCanvas, fontBig, 29, 30, orange, str(len(ipErrors)))
            else:
                graphics.DrawText(offscreenCanvas, fontBig, 29, 30, red, str(len(ipErrors)))
            ...
            for bug in ipErrors:
                ...
                graphics.DrawText(offscreenCanvas, fontSmall, 2, 10, white, "INP" )
                graphics.DrawText(offscreenCanvas, fontSmall, 18, 12, white,  bug['error_class'] )
            ...
            for bug in newErrors:
                ...
                drawSquare(offscreenCanvas,green)
                graphics.DrawText(offscreenCanvas, fontSmall, width-(6*len(bug['severity'])), 7, red ,bug['severity'] )
                graphics.DrawText(offscreenCanvas, fontSmall, 2, 10, white, "NEW" )
                graphics.DrawText(offscreenCanvas, fontSmall, 18, 12, white,  bug['error_class'] )
                graphics.DrawText(offscreenCanvas, fontSmall, 2, 20, white,bug['message'] )
                graphics.DrawLine(offscreenCanvas, 0, 21, width, 21, white)
                graphics.DrawText(offscreenCanvas, fontSmall, 2, 30, white,bug['context'] )

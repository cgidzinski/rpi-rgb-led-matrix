    ...
    def Run(self):
        ...
        def githubOverview(offscreenCanvas):
            ...
            for pr in prs:
                ...
                if pr['approvals'] == 0: graphics.DrawText(offscreenCanvas, fontBig, 255-(5*24), 27, red ,"["+str(pr['approvals'])+"/2]" )
                if pr['approvals'] == 1: graphics.DrawText(offscreenCanvas, fontBig, 255-(5*24), 27, orange ,"["+str(pr['approvals'])+"/2]" )
                if pr['approvals'] >= 2: graphics.DrawText(offscreenCanvas, fontBig, 255-(5*24), 27, green ,"["+str(pr['approvals'])+"/2]" )

    ...
    def Run(self):
        ...
        while True:
            ...
            for x in xrange(0,len(textTop)):
                lenTopA = graphics.DrawText(offscreenCanvas, fontBig, posA+(totalOffsetTop), 11, textTop[x][3], textTop[x][0])
                lenTopB = graphics.DrawText(offscreenCanvas, fontBig, posA+(totalOffsetTop), 22, textTop[x][3], textTop[x][1] +" ("+ textTop[x][2]+")")
                if lenTopA > lenTopB:
                    totalOffsetTop += (lenTopA+8)
                else:
                    totalOffsetTop += (lenTopB+8)
